        ...
        <div class="container-fluid">
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                            <h2>TỰ ĐỘNG ĐĂNG BÀI LÊN TƯỜNG BẠN BÈ</h2>
                        </div>
                        <div class="body">
                            <form id="wizard_with_validation" method="POST" action="<?= SITE_URL ?>/ban-be-facebook/dang-len-tuong-ban-be">
                                ...
                                <h2 class="card-inside-title" id="title-pause" style="margin-bottom: 0px;">Số bài đăng trong một ngày: 15 bài</h2>
                                <small class="col-red" style="margin-bottom: 20px;">Để tránh bị khóa tài khoản thì bạn nên để đăng 15-20 bài một ngày thôi</small>
                                <div id="pause"></div>
                                ...
                                <h2 class="card-inside-title" id="title-pause-time">Thời gian nghỉ sau khi đăng xong 20 bài: 14 tiếng</h2>
...
<script type="text/javascript">
    ...
    pauseTime.noUiSlider.on('update', function () {
        ...
        $("#title-pause-time").html('Thời gian nghỉ sau khi đăng xong '+$('input[name="pause"]').val()+' bài: '+parseInt(val)+' tiếng');
        // Update thời gian hoàn thành
        calcTimeDone();
    });
    ...
    pause.noUiSlider.on('update', function () {
        ...
        $("#title-pause").html('Số bài đăng trong 1 ngày: '+parseInt(val)+' bài');
        $("#title-pause-time").html('Thời gian nghỉ sau khi đăng xong '+parseInt(val)+' bài: '+$('input[name="pause-time"]').val()+' tiếng');
        // Update thời gian hoàn thành
        calcTimeDone();
    });
    ...
    $("#select-facebook-account").on('changed.bs.select', function () {
        ...
        getAccessTokenByFacebookID($id, function (res) {
            if(!res) {showAlert('error', 'Có lỗi xảy ra trong quá trình lấy bạn bè');return;}
            $access_token = res.access_token;
            getAllFriendOfUser($id,$access_token, function (res) {
                $(".loading").html('');
                if(res.data.length <= 0) {showAlert('error', 'Tài khoản này không có bạn bè');return;}
                $(".dataTable").show();
                $("#all-users").html('');
                $.each(res.data, function (key,val) {
                    ...
                    $("#input-users").append('<input type="checkbox" name="users[]" data-id="'+val.id+'" class="filled-in" value="'+val.id+'" style="display:none" checked="checked">');
                });
