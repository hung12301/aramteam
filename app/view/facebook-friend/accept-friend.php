        ...
        getAccessTokenByFacebookID($accountID,function ($data) {
            ...
            getAllFriendRequest($access_token, function(res) {
                ...
                $.each(res.data, function(key,val){
                    $html = '';
                    $html += '<tr>';
                    $html += '<td><input type="checkbox" id="'+val.uid+'" class="filled-in select-user" name="select-user" value="'+val.uid+'" checked="checked"><label for="'+val.uid+'" style="height:9px;"></label></td>';
                    $html += '<td>'+val.uid+'</td>';
